...
impl SimpleAccount {
    pub fn new<P: AsRef<Path>>(
        ...
        let account_addr = Self::create2addr(owner_addr, salt);
        //todo: move entry point to wallet setting
        let entry_point = match entry_point {
            Some(addr) => addr,
            None => Address::from_str("0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789").unwrap(),
        };
...
mod test {
    ...
    pub fn test_store_sign() {
        ...
        let account2 = SimpleAccount::load_account(&path, &addr_str);

        let user_op = UserOperation::new();
        let sig = account.sign(&user_op, "123456789");
