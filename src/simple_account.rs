...
impl SimpleAccount {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        chain_id: U256,
        password: &str,
    ) -> Result<SimpleAccount, MesonWalletError> {
        let signing_key = match SigningKey::from_bytes(owner_sk) {
            Ok(s) => s,
            Err(e) => return Err(MesonWalletError::MesonWalletError(e.to_string())),
        };
        ...
        Erc4337Wallet::encrypt_key(key_dir, &mut rng, owner_sk, password)?;
    ...
    fn account_factory_address<P: AsRef<Path>>(
        ...
        let a = match value["simple_account"][chain_id]["ACCOUNT_FACTORY_ADDRESS"].as_str() {
            Some(a) => a,
            None => {
    ...
    fn account_impl_address<P: AsRef<Path>>(
        ...
        let a = match value["simple_account"][chain_id]["ACCOUNT_IMPLEMENTATION"].as_str() {
            Some(a) => a,
            None => {
                return Err(MesonWalletError::MesonWalletError(
                    "unsupported chain".to_string(),
...
impl Account for SimpleAccount {
    fn create_init_code<P: AsRef<Path>>(
        ...
        let af_addr = match Address::from_str(&af_addr) {
            Ok(a) => a,
            Err(e) => return Err(MesonWalletError::ConfigFileError(e.to_string())),
        };
    ...
    fn sign<P: AsRef<Path>>(
        ...
        let signing_key = match SigningKey::from_bytes(&sk) {
            Ok(s) => s,
            Err(e) => return Err(MesonWalletError::SigningError(e.to_string())),
        };
        let owner_wallet: Wallet<SigningKey> = signing_key.into();

        let signature: Signature = match block_on(owner_wallet.sign_message(op_hash)) {
            Ok(s) => s,
            Err(e) => return Err(MesonWalletError::SigningError(e.to_string())),
        };
        Ok(signature.to_vec())
    ...
    fn delete_account<P: AsRef<Path>>(
        ...
        let addr_str = "0x".to_owned() + &hex::encode(self.address);
        let account_path = key_store_path
            .as_ref()
            .join("simple_account")
            .join(addr_str);
...
mod test {
    ...
    impl SimpleAccount {
        ...
        pub fn verify(&self, user_op: &UserOperation, signature: &[u8]) -> bool {
            ...
            let op_hash = keccak256(AbiEncode::encode((user_op.hash(), self.entry_point)));
    ...
    pub fn test_store_sign() {
        ...
        let account2 = SimpleAccount::load_account(&wallet.key_store_path, &addr_str).unwrap();
