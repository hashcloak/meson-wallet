...
impl BLSAccount {
    pub fn new<P: AsRef<Path>>(
        ...
        let address: Address = Self::bls_create2addr(pk_sol, salt);
        ...
        let key_dir = account.key_store_path.join("key");
        Erc4337Wallet::encrypt_key(key_dir, &mut rng, sk.to_bytes(), password).unwrap();

        //store account settings
        let account_dir = account.key_store_path.join("account");
    ...
    pub fn verify(&self, user_op: &UserOperation, signature: &[u8]) -> bool {
        ...
        if Address::is_zero(&self.aggregator) {
            let entry_point_bytes = AbiEncode::encode(self.aggregator);
            ...
            let aggregator_bytes = AbiEncode::encode(self.aggregator);
            user_op_hash = keccak256(AbiEncode::encode((
                user_op.hash(),
                self.entry_point,
                self.aggregator,
    ...
    pub fn bls_create2addr(bls_pk: SolPublicKey, salt: U256) -> Address {
...
impl Account for BLSAccount {
    ...
    fn sign(&self, user_op: &UserOperation, password: &str) -> Vec<u8> {
        ...
        let key_dir = self.key_store_path.join("key");
...
mod test {
    ...
    pub fn test_store_sign_bls() {
        ...
        let sig = bls_account.sign(&user_op, "123456789");
